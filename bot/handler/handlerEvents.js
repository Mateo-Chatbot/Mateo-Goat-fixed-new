...
function getText(type, reason, time, targetID, lang) {
	...
	else if (type == "maintainceMode")
		return utils.getText({ lang, head: "handlerEvents" }, "maintainceMode");
}
...
function createGetText2(langCode, pathCustomLang, prefix, command) {
	...
	if (command.langs || customLang || {}) {
		getText2 = function (key, ...args) {
			...
			return lang || `❌ Can't find text in mateo on language "${langCode}" for ${commandType} "${commandName}" with key "${key}"`;
		};
	}
	return getText2;
}
